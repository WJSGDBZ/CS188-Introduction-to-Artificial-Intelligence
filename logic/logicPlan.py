...
def sentence1() -> Expr:
    ...
    a_and_b = A | B
    na__iff__nb_or_c = ~A % (~B | C)
    na_or_nb_or_c = disjoin([~A, ~B, C])

    return conjoin([a_and_b, na__iff__nb_or_c, na_or_nb_or_c])
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    c__iff__b_or_d = C % (B | D)
    a__imp__nb_and_nd = A >> (~B & ~D)
    not__b_and_nc__imp__a = (~(B & ~C)) >> A
    nd__imp__c = ~D >> C

    return conjoin([c__iff__b_or_d, a__imp__nb_and_nd, not__b_and_nc__imp__a, nd__imp__c])
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    PacmanAlive_str = "PacmanAlive"
    PacmanBorn_str = "PacmanBorn"
    PacmanKilled_str = "PacmanKilled"

    PacmanAlive_0 = PropSymbolExpr(PacmanAlive_str, time=0)
    PacmanAlive_1 = PropSymbolExpr(PacmanAlive_str, time=1)
    PacmanBorn_0 = PropSymbolExpr(PacmanBorn_str, time=0)
    PacmanKilled_0 = PropSymbolExpr(PacmanKilled_str, time=0)

    # Pacman is alive at time 1 if and only if he was alive at time 0 and he was not killed at time 0
    # or he was not alive at time 0 and he was born at time 0.
    EventA = PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0))

    # At time 0, Pacman cannot both be alive and be born.
    EventB = ~(PacmanAlive_0 & PacmanBorn_0)

    # Pacman is born at time 0.
    EventC = PacmanBorn_0

    return conjoin([EventA, EventB, EventC])
    "*** END YOUR CODE HERE ***"
